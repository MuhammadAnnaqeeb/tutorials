...
class EstateProperty(models.Model):
    ...
    date_availability = fields.Date(
        string="Available from", default=three_months_later, copy=False
    )
    ...
    offer_ids = fields.One2many(
        "estate.property.offer", inverse_name="property_id", string="Property Offers"
    )
    ...
    def _check_selling_price(self):
        for prop in self:
            if (
                ...
                and float_compare(
                    prop.selling_price, 0.9 * prop.expected_price, precision_digits=2
                )
                == -1
            ):
                raise ValidationError(
                    "Selling Price Can not be less than 90% of expected price"
                )
